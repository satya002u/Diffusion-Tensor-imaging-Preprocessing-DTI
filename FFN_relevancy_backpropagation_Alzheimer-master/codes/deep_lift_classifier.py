...
# main_dir = "/media/satya/win" # for Linux
main_dir = "E:/"              # for Windows   
...
contr_score_type = ['grad_input']
classification_list=['cn-ad', 'cn-emci', 'emci-ad']
classification_list=['cn-ad']

for contr_score in contr_score_type:

    for classification_type in classification_list:
        # if classification_type == 'cn-ad':
        #     data_dir=main_dir + '/data_baseline/ztrial/neural_net/data/data_cn-ad_nMatric/'
        # elif classification_type == 'cn-emci':
        #     data_dir=main_dir + '/data_baseline/ztrial/neural_net/data/data_cn-emci_nMatric/'
        # elif classification_type == 'cn-lmci':
        #     data_dir=main_dir + '/data_baseline/ztrial/neural_net/data/data_cn-lmci_nMatric/'
        # elif classification_type == 'emci-ad':
        #     data_dir=main_dir + '/data_baseline/ztrial/neural_net/data/data_emci-ad_nMatric/'
        ...
        mask = np.ones(6786)
        thresholds = generate_compunded_neurons(0.9, 23)
        ...
        for ix, i in enumerate(thresholds):
            ...
            for itr in range(1):
                ...
                # _x_train, _y_train, _x_test, _y_test = data_generator(data_dir, classification_type , frac_train, frac_test)
                if ix == 0:
            ...
            for c in cm:
                _confusion_matrix+=c
            _confusion_matrix=np.divide(_confusion_matrix, 1)  
            ...
            f1 = (2 * tp) / (2 * tp + fp + fn)
            test_acc = (tp+tn)/(tp+tn+fp+fn);print(test_acc)
            ...
            with open(training_logs + 'training_logs_' + classification_type + '.csv', 'a') as out_stream:
                    out_stream.write(str(i)  + ', ' + str(sensitivity) + ', ' + str(specificity) + ', ' + str(f1) + ', ' + str(valacc) + ', ' + str(test_acc) + '\n')
